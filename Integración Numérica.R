...
CG <- function(g,a,b,n){
  ...
  m1 <- matrix(c(1,1,0,0,0,.5555555556,.8888888889,.5555555556,0,0,.3478548451,.6521451549
                 ,.6521451549,.3478548451,0,.236926885,.4786286705,.5688888889,.4786286705,.236926885),5,4)
  #Construimos una matriz con los xi's correspondientes
  m2 <- matrix(c(-sqrt(1/3),sqrt(1/3),0,0,0,.7745966692,0,-.7745966692,0,0,.8611363116,.3399810436,-.3399810436,
                 -.8611363116,0,.9061798459,.5384693101,0,-.5384693101,-.9061798459),5,4)
  ...
  if(a == -1 && b == 1){
    #Llenamos el vector w con sus valores correspondientes de la matriz m1 dependiendo el valor de n
    #Llenamos el vector x con los valores de f(x) evaluadas en los x de la matrix m2
    for(i in 1:n){
      w[i] <- m1[i,n-1]
      x[i] <- f(m2[i,n-1])
    }
    ...
    #multiplicamos entrada por entrada de los vectores x y w y sumamos a esto lo multuplicamos por el 
